...
// struct alignment* aln_alloc(struct alignment* aln)/*{{{*/ /*original code*/
// {
// 	int i;
...
// 	return aln;
// }/*}}}*/
struct alignment* aln_alloc(struct alignment* aln)/*{{{*/ /*modified from 2010-10-01*/
{
	int i;
	aln = (struct alignment*) malloc(sizeof(struct alignment));
	aln->s = malloc(sizeof(int*) * (numseq ));
	aln->seq = malloc(sizeof(char*) * (numseq ));
	aln->gpo = malloc(sizeof(float*) * (numseq )); /*added 2010-10-01*/
	aln->gpe = malloc(sizeof(float*) * (numseq )); /*added 2010-10-01*/
	aln->tgpe = malloc(sizeof(float*) * (numseq )); /*added 2010-10-01*/
	aln->ft =  malloc(sizeof(struct feature* ) * (numseq));
	aln->si  =  malloc(sizeof(struct sequence_information* ) * (numseq));
	aln->sl = malloc(sizeof(unsigned int) * (numprofiles));
	aln->sip = malloc(sizeof(unsigned int*)* numprofiles);
	aln->nRes = malloc(sizeof(unsigned int*)* numprofiles); /*added 2010-10-19*/
	...
	for(i =0;i < numseq;i++){
		...
		aln->nsip[i] = 1;
		aln->sip[i][0] = i;
        aln->gpo[i] = 0; /*added 2010-10-01*/
        aln->gpe[i] = 0; /*added 2010-10-01*/
        aln->tgpe[i] = 0; /*added 2010-10-01*/
	}
	return aln;
}/*}}}*/


// void free_aln(struct alignment* aln)/*{{{*/ /*original code*/
// {
// 	int i;
// 	for (i = numseq;i--;){
// 		free(aln->s[i]);
...
// 	free(aln);
// }/*}}}*/
void free_aln(struct alignment* aln)/*{{{*/ /*modified from 2010-10-01*/
{
	int i;
	for (i = numseq;i--;){
		free(aln->s[i]);
		free(aln->seq[i]);
		free(aln->sn[i]);
        if(aln->gpo[i]){ /*added 2010-10-01*/   
            free(aln->gpo[i]);
        }
        if(aln->gpe[i]){/*added 2010-10-01*/ 
            free(aln->gpe[i]);
        }
        if(aln->tgpe[i]){/*added 2010-10-01*/ 
            free(aln->tgpe[i]);
        }
	}
    ...
    free(aln->gpo); /*added 2010-10-01*/
    free(aln->gpe); /*added 2010-10-01*/ 
    free(aln->tgpe);/*added 2010-10-01*/ 
	free(aln->seq);
	free(aln->s);
	free(aln->sn);
	free(aln->sl);
