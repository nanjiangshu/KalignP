...
extern float FLOAT_ZERO; /*difference between two floating numbers which can be ignored*/
...
struct parameters{
	...
	char* sort;
	char* sub_matrix;
    char* custom_sub_matrix_file; /*added 2014-11-27 by Nanjiang, supply the customer substitution matrix file*/
	char* print_tree;
	char* print_svg_tree;
    int seq_type; /*added 2014-11-27 by Nanjiang, force the seq_type, 0: protein, 1: dna, 2: rna, -1: not set (default)*/
	float gpo;
	float gpe;
	float tgpe;
	float secret;
...
// struct alignment{   /*original struct alignment*/
// 	//struct node** seq;
// 	struct feature** ft;
...
struct alignment{  /*modified from 2010-09-29*/
    /* sip  :
     * nsip : 
     * e.g. for A, B, C and D four sequences.
     * if the tree is (((A,B), C), D), (A,B) will generate a consensus (idx=4), (AB) and C will generate a consensus with idx = 5. and thus nsip[5] = 3, meaning three sequences are included in this consensus*/
	//struct node** seq;
	struct feature** ft;
	struct sequence_info** si;
	unsigned int** nRes;   /*number of residues at each position of a sequence or consensus sequence*/
	unsigned int** sip;   /*index to the original sequences that are involved in the consensus*/
	unsigned int* nsip;  /*number of sequences involved in the consensus*/
	unsigned int* sl;  /*length of the sequence*/
	unsigned int* lsn; /*length of annotation lines*/
    float **gpo; /*position specific gapopen array, added 2010-09-29*/
    float **gpe;  /*position specific gapextension array, added 2010-09-29*/ 
    float **tgpe;/*position specific terminal gapextension array, added 2010-09-29*/ 
	int** s;      /*sequences in digit mapped by aacode*/
	char**seq;    /*sequences in text*/
	char** sn;    /*annotation lines*/
};
...
struct states{
	float a;   /*aligned*/
	float ga;  /*gap in sequence a */ 
	float gb;  /*gap in sequence b*/ 
	float x;
};
...
float* update_new2(float **profile, int* path, struct alignment* aln, int a, int b, int c);
...
int IsInCharSet(const char ch, const char *charSet, int n );
char *SpanExcluding(const char* strForSpan,char* strAfterSpan, const char charSet[] );
int fgetline(FILE* fp, char* line, int max);
int checkfilestream(FILE *fp, const char* filename, const char *mode, int isAssert);
