...
int IsInCharSet(const char ch, const char *charSet, int n /*= 0 */)/*{{{*/
    /*****************************************************************************
...
int checkfilestream(FILE *fp, const char* filename, const char *mode, int isAssert /*=false*/)/*{{{*/
{
    if( fp == 0)
    {
...
/*}}}*/
int ReadSMatrix(const char *filename, short **S, char *alphabet)/*{{{*/
/*****************************************************************************
    ...
    ****************************************************************************/
{
    int linesize;
    int maxline = 300;
    char *line = malloc(sizeof(char)*maxline+1);

    char *pch;
    int i,j;
    FILE* fpout = NULL;
    fpout = fopen(filename,"r");
    checkfilestream(fpout,filename, "r", 1);
...
float** read_matrix_new(float** subm,struct parameters* param) {/*{{{*/
    /*updated 2014-11-27 by Nanjiang, this function also accept user input substitution
     * matrix*/
    int i,j;
    int m_pos = 0;
    short *matrix_pointer = 0;

    char matrix_alphabet[] = "ABCDEFGHIKLMNPQRSTVWXYZ";
    int size_sub_matrix = strlen(matrix_alphabet);
    short *user_matrix = 0;
    if (param->custom_sub_matrix_file){
        short **Mtmp = 0;
        Mtmp = malloc(sizeof(short*) * size_sub_matrix+10);
        for (i = 0; i < size_sub_matrix; i++){
            Mtmp[i] = malloc(sizeof(short)*size_sub_matrix+10);
        }

        char *user_alphabet = malloc(sizeof(char)*size_sub_matrix+10);
        int nvalue = ReadSMatrix(param->custom_sub_matrix_file, Mtmp, user_alphabet);
        user_matrix = malloc(sizeof(short)*size_sub_matrix*size_sub_matrix);
        m_pos = 0;
        for (i = 0; i< size_sub_matrix; i++){
            for (j=0 ; j <=i; j++){
                user_matrix[m_pos] = Mtmp[i][j]*10;
                m_pos ++;
            }
        }
        free(user_alphabet);
...
/*E*/  -10,  10, -40,  20,  50,
/*F*/  -20, -30, -20, -30, -30,  60,
/*G*/    0, -10, -30, -10, -20, -30,  60,
/*H*/  -20,   0, -30, -10,   0, -10, -20,  80,
/*I*/  -10, -30, -10, -30, -30,   0, -40, -30,  40,
/*K*/  -10,   0, -30, -10,  10, -30, -20, -10, -30,  50,
/*L*/  -10, -40, -10, -40, -30,   0, -40, -30,  20, -20,  40,
/*M*/  -10, -30, -10, -30, -20,   0, -30, -20,  10, -10,  20,  50,
/*N*/  -20,  30, -30,  10,   0, -30,   0,  10, -30,   0, -30, -20,  60,
/*P*/  -10, -20, -30, -10, -10, -40, -20, -20, -30, -10, -30, -20, -20,  70,
/*Q*/  -10,   0, -30,   0,  20, -30, -20,   0, -30,  10, -20,   0,   0, -10,  50,
/*R*/  -10, -10, -30, -20,   0, -30, -20,   0, -30,  20, -20, -10,   0, -20,  10,  50,
/*S*/   10,   0, -10,   0,   0, -20,   0, -10, -20,   0, -20, -10,  10, -10,   0, -10,  40,
/*T*/    0, -10, -10, -10, -10, -20, -20, -20, -10, -10, -10, -10,   0, -10, -10, -10,  10,  50,
/*V*/    0, -30, -10, -30, -20, -10, -30, -30,  30, -20,  10,  10, -30, -20, -20, -30, -20,   0,  40,
/*W*/  -30, -40, -20, -40, -30,  10, -20, -20, -30, -30, -20, -10, -40, -40, -20, -30, -30, -20, -30, 110,
/*X*/    0, -10, -20, -10, -10, -10, -10, -10, -10, -10, -10, -10, -10, -20, -10, -10,   0,   0, -10, -20, -10,
/*Y*/  -20, -30, -20, -30, -20,  30, -30,  20, -10, -20, -10, -10, -20, -30, -10, -20, -20, -20, -10,  20, -10, 70,
/*Z*/  -10,  10, -30,  10,  40, -30, -20,   0, -30,  10, -30, -10,   0, -10,  30,   0,   0, -10, -20, -30, -10, -20,  40
    ...
    if (param->custom_sub_matrix_file){ /*using user defined matrix*/
        matrix_pointer = user_matrix;
        gpo = (float)55.0;
        gpe = (float)8.0;
        tgpe = (float)1.0;
    }else{
...
{/*path[0]: max (len_a, len_b) path[i] ended with 3*/
	int c;
	int i;
	int posa = 0;
	int posb = 0;
